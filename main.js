...
app.post('/upload-item', uploadFile.single('file'), rutasProtegidas,  (req, res)=> {
    ...
    connection.query('INSERT INTO Items (name, author, genere, description, copyright, uploadDate, item) VALUES(?, ?, ?, ?, ?, ?, ?)', [req.body.name, req.body.author, req.body.genere, req.body.description, true, req.body.uploadDate, req.file.filename], (err, response)=>{
        if(err){
            res.status(500).end();
        }else{
            console.log("Item subida")
